...
export function ComplianceWizard() {
  ...
  if (result) { const canPrint = Boolean(result.guidance.grounded && !result.guidance.insufficient_evidence && !result.guidance.needs_clarification && !error && !busy && reportGeneratedAt); return <>{canPrint && createPortal(<CompliancePrintReport ref={printReportRef} profile={result.profile} guidance={result.guidance} generatedAt={reportGeneratedAt!} />, document.body)}<section className="wizard journey-result" aria-label={t('journeyResultAria')}><div className="result-heading"><div><p className="eyebrow">{t('groundedGuidance')}</p><h2>{t('journeyTitle')}</h2><p>{t('journeyDescription')}</p></div><div className="result-actions">{canPrint && <button type="button" onClick={event => { if (printReportRef.current) printComplianceReport(printReportRef.current, event.currentTarget, t('printReportTitle')); }} aria-label={t('downloadPrintAria')}>{t('downloadPrint')}</button>}<button type="button" className="secondary" onClick={reset}>{t('startOver')}</button></div></div>{updatingResult && <p className="answer-update" aria-live="polite">{t('updatingAnswers')}</p>}<section className="journey-category journey-profile-summary" aria-labelledby="journey-product-profile"><h2 id="journey-product-profile">{t('profileTitle')}</h2><dl><dt>{t('profileProduct')}</dt><dd>{result.profile.product_description}</dd><dt>{t('profilePower')}</dt><dd>{label(result.profile.power_type)}</dd><dt>{t('profileAge')}</dt><dd>{label(result.profile.intended_age_group)}</dd><dt>{t('profileStage')}</dt><dd>{label(result.profile.application_stage)}</dd><dt>{t('profileGoal')}</dt><dd>{label(result.profile.goal)}</dd></dl><p>{t('profileNote')}</p></section><ChatMessage role="assistant" text={result.guidance.answer} response={result.guidance} presentation="compliance-journey" />{error && <p role="alert" className="error">{error} <button type="button" onClick={() => void generate()}>{t('retry')}</button></p>}<p className="report-disclaimer">{t('reportDisclaimer')}</p><p className="wizard-disclaimer">{result.guidance.disclaimer}</p></section></>; }
