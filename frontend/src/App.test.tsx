...
it('submits the original English built-in question from a translated suggestion exactly once',async()=>{await renderReady();const user=userEvent.setup();await user.selectOptions(screen.getByLabelText('Language'),'hi');await user.click(screen.getByRole('button',{name:'बैटरी से चलने वाले खिलौने पर कौन सा मानक लागू होता है?'}));expect(await screen.findByText(a.answer)).toBeInTheDocument();const calls=(globalThis.fetch as ReturnType<typeof vi.fn>).mock.calls.filter(([url])=>String(url).includes('/api/chat'));expect(calls).toHaveLength(1);const payload=JSON.parse(String((calls[0][1] as RequestInit).body));expect(payload.question).toBe('Which standard applies to a battery-operated toy?');expect(payload.response_language).toBe('hi')});
