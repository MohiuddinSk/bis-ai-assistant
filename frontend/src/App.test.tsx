...
const a={answer:'IS 15644 is the primary standard.',grounded:true,insufficient_evidence:false,evidence_count:2,citations:[{citation_id:'S1',source_filename:'manual.pdf',page_start:4,page_end:4,chunk_id:'secret',excerpt:'IS 15644 applies.'}],model:'test',generation_mode:'llm' as const,disclaimer:'Verify'};const rep=(x:unknown)=>({ok:true,json:async()=>x});const mock=(chat:unknown=a,health:unknown={status:'ready'})=>vi.stubGlobal('fetch',vi.fn((u:string)=>Promise.resolve(rep(u.includes('health')?health:chat))));beforeEach(()=>{localStorage.clear();document.documentElement.lang='';vi.restoreAllMocks();mock()});afterEach(()=>{cleanup();vi.restoreAllMocks()});const renderReady=async()=>{render(<App/>);await screen.findByRole('status')};
it('renders welcome',async()=>{await renderReady();expect(screen.getByText('Suggested questions')).toBeInTheDocument()});
it('defaults safely to English and sets the document language',async()=>{await renderReady();expect(screen.getByLabelText('Language')).toHaveValue('en');expect(document.documentElement.lang).toBe('en');expect(screen.getByRole('heading',{name:'BIS AI Assistant'})).toBeInTheDocument()});
it('translates visible UI when Hindi or Marathi is selected',async()=>{await renderReady();const user=userEvent.setup();const selector=screen.getByLabelText('Language');await user.selectOptions(selector,'hi');expect(screen.getByLabelText('भाषा')).toHaveValue('hi');expect(screen.getByText('सुझाए गए प्रश्न')).toBeInTheDocument();expect(screen.getByRole('button',{name:'भेजें'})).toBeInTheDocument();expect(document.documentElement.lang).toBe('hi');await user.selectOptions(screen.getByLabelText('भाषा'),'mr');expect(screen.getByText('सुचवलेले प्रश्न')).toBeInTheDocument();expect(screen.getByRole('button',{name:'पाठवा'})).toBeInTheDocument();expect(document.documentElement.lang).toBe('mr')});
it('persists the selected language and rejects invalid stored values',async()=>{await renderReady();await userEvent.setup().selectOptions(screen.getByLabelText('Language'),'hi');expect(localStorage.getItem('bis-assistant-language')).toBe('hi');cleanup();render(<App/>);await screen.findByRole('status');expect(screen.getByLabelText('भाषा')).toHaveValue('hi');cleanup();localStorage.setItem('bis-assistant-language','invalid');render(<App/>);await screen.findByRole('status');expect(screen.getByLabelText('Language')).toHaveValue('en');expect(document.documentElement.lang).toBe('en')});
it('displays Hindi built-in labels while submitting their canonical English question exactly once',async()=>{await renderReady();const user=userEvent.setup();const label='बैटरी से चलने वाले खिलौने पर कौन सा मानक लागू होता है?';await user.selectOptions(screen.getByLabelText('Language'),'hi');await user.click(screen.getByRole('button',{name:label}));expect(await screen.findByText(a.answer)).toBeInTheDocument();expect(document.querySelector('.message.user')).toHaveTextContent(label);const calls=(globalThis.fetch as ReturnType<typeof vi.fn>).mock.calls.filter(([url])=>String(url).includes('/api/chat'));expect(calls).toHaveLength(1);const payload=JSON.parse(String((calls[0][1] as RequestInit).body));expect(payload.question).toBe('Which standard applies to a battery-operated toy?');expect(payload.response_language).toBe('hi')});
it('displays Marathi built-in labels while preserving the canonical English payload',async()=>{await renderReady();const user=userEvent.setup();const label='बॅटरीवर चालणाऱ्या खेळण्याला कोणते मानक लागू होते?';await user.selectOptions(screen.getByLabelText('Language'),'mr');await user.click(screen.getByRole('button',{name:label}));await screen.findByText(a.answer);expect(document.querySelector('.message.user')).toHaveTextContent(label);const calls=(globalThis.fetch as ReturnType<typeof vi.fn>).mock.calls.filter(([url])=>String(url).includes('/api/chat'));expect(JSON.parse(String((calls[0][1] as RequestInit).body)).question).toBe('Which standard applies to a battery-operated toy?')});
...
it('Enter sends manually typed text unchanged',async()=>{render(<App/>);const question='Can I use my own wording?';await userEvent.setup().type(screen.getByLabelText(/ask a question/i),`${question}{Enter}`);expect(await screen.findByText(a.answer)).toBeInTheDocument();expect(document.querySelector('.message.user')).toHaveTextContent(question)});
...
it('renders accessible suggested standard replies and sends the chosen standard once',async()=>{
 ...
 expect(payload.assistant_context).toEqual(context);
 expect(document.querySelectorAll('.message.user')[1]).toHaveTextContent('IS 15644');
